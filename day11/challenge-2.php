...
class Monkey
{
    ...
    public function processItems(array &$allMonkeys): void
    {
        foreach ($this->items as $index => $worryLevel) {
            ...
            switch ($operator) {
                ...
                case '*':
                    $worryLevel = (int)$value1 * $value2;
                    $worryLevel = $this->findLowerValueWithSameProperties($allMonkeys, $worryLevel);
     ...
     * @return int
     */
    private function findLowerValueWithSameProperties(array $allMonkeys, int $worryLevel): int
    {
        $a = [];

        foreach ($allMonkeys as $monkey) {
            $divisibleBy = (int)$monkey->test[2];
            $isDivisible = (($worryLevel % $divisibleBy) === 0);
            $a[$divisibleBy] = $isDivisible;
        }
        ...
        while(true) {
            ...
            foreach ($a as $divisibleBy => $isDivisible) {
                if ($isDivisible) {
                    if (($c % $divisibleBy) !== 0) {
                        $found = false;
                        break;
...
foreach ($allMonkeys as $monkeyIdx => $monkey) {
    print "Inspection count for monkey $monkeyIdx: {$monkey->getInspectionCount()}\n";
