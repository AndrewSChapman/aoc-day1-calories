...
$count = findWhollyOverlappingItemCount($assignmentData);

print "Fully overlapping count: $count\n";

function findWhollyOverlappingItemCount(array $assignmentData): int
{
    $numFullyOverlappingItems = 0;

    foreach ($assignmentData as $rangeData) {
        if ((($rangeData[0] <= $rangeData[2]) && ($rangeData[1] >= $rangeData[3])) ||
            (($rangeData[2] <= $rangeData[0]) && ($rangeData[3] >= $rangeData[1]))) {
            $numFullyOverlappingItems++;
        }
    }

    return $numFullyOverlappingItems;
}
